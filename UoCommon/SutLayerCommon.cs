...
{
    public class SutLayerCommon<Conn, Trans, Cmd, Param>
        ...
        where Param : DbParameter, new ()
    {
        ...
        Db.DbCommon<Conn, Trans, Cmd, Param> DbCommon;

        string provider;


        public SutLayerCommon(string connStr)
        {
            DbCommon = new Db.DbCommon<Conn, Trans, Cmd, Param>(connStr);
        }
        ...
        public bool CheckConnect()
        {
            ...
            if (DbCommon.openConnection().State == ConnectionState.Open)
            {
        ...
        public DataTable getSoftware()
        {
            ...
            dt = DbCommon.Select("UO_GET_SOFTWARE", null, true, false, out errmess);
        ...
        public DataTable getStatusDevicesUpdate(int UpdId)
        {
            ...
            lParams.Add(new Param { Value = UpdId, DbType = DbType.Int32, ParameterName = "updid" });

            DataTable dt;

            dt = DbCommon.Select("UO_GET_DEVICES_UPDATE", lParams.ToArray(), true, false, out errmess);
            ...
            //        if (FromDate != null)
            //        {
            //            lParams.Add(new NpgsqlParameter { ParameterName = "fromdate", Value = FromDate.Value, DbType = DbType.DateTime });
            //            // lParams.Add(new DbParameter("FromDate", SqlDbType.SmallDateTime) { Value = FromDate.Value });
            //        }
            //        if (isForceRefresh)
            //        {
            ...
            //        if (DevIds != null && DevIds.Count > 0)
            //        {
            //            lParams.Add(new NpgsqlParameter  { ParameterName = "DevIds", Value = DevIds.ToArray(), NpgsqlDbType = NpgsqlTypes.NpgsqlDbType.Array});
            //        }
            ...
            //  //  lParams.Add(new DbParameter("UpdId", SqlDbType.Int) { Value = updtId });
            ...
            //   // lParams.Add(new DbParameter("DuIds", duids));
